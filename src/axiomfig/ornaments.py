...
from axiomfig.layout import get_figure_layout, outer_panel_bbox, register_figure_ornament
...
def requested_legend_height_pt(axis: Axes) -> float:
    ...
    if len(handles) <= 1:
        return 0.0
    probe = axis.legend(handles, labels, loc="lower right", **_legend_kwargs(len(handles)))
    axis.figure.canvas.draw()
    height = probe.get_window_extent(axis.figure.canvas.get_renderer()).height
    probe.remove()
    return height * 72.0 / axis.figure.dpi
