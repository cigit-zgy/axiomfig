...
- Non-default visual or positional request only: read `references/element-contracts/index.md`, then
  at most one routed element topic when possible. Translate low-level plotting wording into the
  underlying semantic goal, preserve the scientific grammar, and use only a real `AVAILABLE`
  surface. For `INTERNAL_ONLY`, `PLANNED`, or `NOT_SUPPORTED`, preserve the goal without inventing
  Figure Intent fields, coordinates, physical numbers, backend switches, or plotting parameters.
  Keep deterministic defaults for every unrelated element.
