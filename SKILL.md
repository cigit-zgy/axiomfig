...
- Treat an Outer Panel Footprint as the top-level GridSpec slot and register its Primary Axes, Auxiliary Axes, and panel-owned artists before ornament finalization. A colorbar is Auxiliary Axes contained inside the same footprint; it cannot expand the footprint or compress a peer. Anchor 11 pt bold labels only to the footprint upper-left plus physical-point translation.
