...
def build_four_panel() -> Figure:
    ...
    axes[1].scatter(observed, predicted, s=15)
    axes[1].plot([0, 28], [0, 28], color="0.25", linestyle="--")
    axes[1].set(xlim=(0, 28), ylim=(0, 28), xlabel="Observed", ylabel="Predicted")

    positions = np.arange(3)
    width = 0.35
    axes[2].bar(positions - width / 2, [0.72, 0.67, 0.61], width, label="ASM")
    axes[2].bar(positions + width / 2, [0.84, 0.76, 0.71], width, label="Hybrid")
