...
Required consumers include deterministic nice-axis, open/filled/categorical tick, face-alpha/opaque-edge, exact bar-width, redundant-series, legend, registered outer-footprint, panel-label, anatomy validation, output-margin, and colorbar-layout helpers. Registered grids must be built through `layout.py`; legends, panel labels, and colorbars must be finalized through `ornaments.py`. Do not add per-template coordinates, font sizes, stroke widths, edge colors, fill alpha, marker sizes, bar widths, line-cycle overrides, tick geometry, or export crop policies.
