...
def test_gallery_projection_expands_only_the_public_mantel_template() -> None:
    ...
    assert set(counts) == {spec.template_id for spec in public_template_specs()}
    assert counts["association/mantel"] == 4
    assert all(count == 1 for template, count in counts.items() if template != "association/mantel")
    assert tuple(
